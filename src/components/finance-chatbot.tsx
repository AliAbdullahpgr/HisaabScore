...
// Animation variants for chatbot
...
export function FinanceChatbot({ transactions }: FinanceChatbotProps) {
  ...
  const chatWindowTransition = prefersReducedMotion
    ? ({ duration: 0.05 } as const)
    : ({ duration: 0.3, ease: [0.4, 0, 0.2, 1] as const } as const);

  const messageTransition = prefersReducedMotion
    ? ({ duration: 0.05 } as const)
    : ({ duration: 0.2, ease: [0.4, 0, 0.2, 1] as const } as const);
    ...
    <AnimatePresence mode="wait">
      {isOpen && !isMinimized && (
        <motion.div
          ...
          animate="visible"
          exit="exit"
          transition={chatWindowTransition}
          className="fixed bottom-6 right-6 z-50"
        >
          <Card className="w-[420px] h-[75vh] max-h-[800px] min-h-[500px] shadow-2xl flex flex-col border-2 overflow-hidden">
            {/* Header */}
